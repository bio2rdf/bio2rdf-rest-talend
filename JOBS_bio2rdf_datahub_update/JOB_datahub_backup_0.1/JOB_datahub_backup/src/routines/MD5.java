...
*/
	    ...
	    /**
	     * getHash(String inFileName) Returns the MD5 Sum of the input file
	     * 
	     * {talendTypes} String
	     * 
	     * {Category} MD5
	     * 
	     * {param} string(row1.filename)
	     * 
	     * {example} getHash(row1.filename)
	     * 
	     */

		public static String gethash(String inStr){
			try {
				return getHashString(new File(inStr));	
			} catch (IOException x){
				System.err.println(x.getMessage());
				return null;
			}
	     ...
	     * {Category} MD5
	     * 
	     * {param} string(row1.input_string) input: The jobname to be throttled.
	     * 
	     * {example} getHashString(row1.inputString) 
	     * 
	     */
	    ...
	    /**
	     * 
	     * getHashString: returns hash of the input string of length n
	     * 
	     * {talendTypes} String
	     * 
	     * {Category} Voltage Data Mask
	     * 
	     * {param} string(row1.input_string, num_chars) input: The jobname to be throttled.
	     * 
	     * {example} getHashStringN("MY SSN") 
	     */
		
		public static String getHashStringN(String s, Integer n){
			MD5 md5 = new MD5();
			md5.update(s);
			return md5.getHashString().substring(0, n);
		}
